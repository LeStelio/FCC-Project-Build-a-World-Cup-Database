...
echo "$($PSQL "SELECT DISTINCT name FROM teams INNER JOIN (SELECT DISTINCT winner_id AS team_id FROM games WHERE round = 'Eighth-Final' AND year = 2014 UNION SELECT DISTINCT opponent_id AS team_id FROM games WHERE round = 'Eighth-Final' AND year = 2014) AS participating_teams ON teams.team_id = participating_teams.team_id")"
